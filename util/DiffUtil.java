        ...
        public void dispatchUpdatesTo(@NonNull ListUpdateCallback updateCallback) {
            ...
            for (int snakeIndex = this.mSnakes.size() - 1; snakeIndex >= 0; snakeIndex--) {
                ...
                for (int i2 = snakeSize - 1; i2 >= 0; i2--) {
                    if ((this.mOldItemStatuses[snake.f40x + i2] & 31) == 2) {
                        batchingCallback.onChanged(snake.f40x + i2, 1, this.mCallback.getChangePayload(snake.f40x + i2, snake.f41y + i2));
                    }
                }
                posOld2 = snake.f40x;
                posNew = snake.f41y;
            ...
            for (int d = 0; d <= dLimit; d++) {
                ...
                while (k <= d) {
                    ...
                    if (!checkInFwd || k < (delta - d) + 1 || k > (delta + d) - 1 || iArr[kOffset + k] < iArr2[kOffset + k]) {
                        ...
                        outSnake.f40x = iArr2[kOffset + k];
                        outSnake.f41y = outSnake.f40x - k;
                ...
                while (k2 <= d) {
                    ...
                    if (checkInFwd || k2 + delta < (-d) || k2 + delta > d || iArr[kOffset + backwardK] < iArr2[kOffset + backwardK]) {
                        ...
                        outSnake2.f40x = iArr2[kOffset + backwardK];
                        outSnake2.f41y = outSnake2.f40x - backwardK;
